...
* **Balanced labels (1:1).** Each row already contains the pre-fix function (`vulnerable_code`) and the post-fix function (`fixed_code`). The converter emits **both** as a matched pair: the pre-fix snippet is labeled `1` (vulnerable) and anchored to the diff's PRE-image line range (read from the *parent* of the fix commit), while the post-fix snippet is labeled `0` (safe) and anchored to the POST-image line range (pinned to the fix commit via `target_commit`). Treating the commit *before* the fix as vulnerable and the commit at the fix as safe (we deliberately **do not** use "the commit after the fix", which is typically an unrelated change) therefore yields a naturally **balanced** dataset with no synthetic resampling — important for an unbiased F2 / recall evaluation.
* **Safe-counterpart integrity (methodology).** The post-fix function is only a *relative* negative (it lacks *that one* CVE, but may still contain a *different* weakness), and a fix commit can bundle unrelated edits so the `label=0` sample differs from its twin for non-security reasons. We therefore (i) **measure** the contamination with `src/scripts/diagnose_safe_counterparts.py`, which pairs each vulnerable sample with its post-fix twin (both embedded inline, no cloning) and reports the fraction of *substantial* changes (>50% of lines reworked — the bundled-edit red flag) and a `changed_line_ratio` histogram; (ii) **inject verified-benign controls** via `src/scripts/mine_benign_functions.py`, which mines functions from files the fix commit did **not** modify (demonstrably not part of any vulnerability fix) and merges them as `label=0`, `sample_subtype="benign_control"` into both the training pool and the VUDENC gate study; and (iii) optionally **use the post-fix pair as a contrastive signal** (`--contrastive`, off by default) so a noisy fixed function weakly perturbs a similarity objective rather than a confident 0/1 label. The full, citable methodology is `docs/SAFE_COUNTERPARTS.md`.
