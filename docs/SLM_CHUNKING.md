...
4. ✅ **Contrastive term (optional)** — implemented in `src/training/trainer.py` as an opt-in supervised-contrastive loss on top of CE (default OFF). Part of the safe-counterpart remedy; see `docs/SAFE_COUNTERPARTS.md`. Multi-class CWE head + `max-pooling` still future.
